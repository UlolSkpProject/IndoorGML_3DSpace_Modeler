    ...
    module IndoorCore
      module PrecisionValidation
        module LvnState
          ...
          # Compatibility readers remain available, but no geometry signature is
          # persisted or calculated. Geometry edits clear lvn_failed through the
          # existing CellSpace observer path.
          def failure_signature(_candidate)
            nil
          end

          def geometry_signature(_candidate)
            nil
          end
          ...
          def set_failed(candidate, failed)
            ...
            if group.respond_to?(:delete_attribute)
              group.delete_attribute(DICTIONARY_NAME, LEGACY_FAILED_SIGNATURE_KEY)
            end
            true
