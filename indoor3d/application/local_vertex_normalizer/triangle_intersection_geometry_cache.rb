...
module ULOL
  module Indoor3DGmlModeler
    module IndoorCore
      ...
      module LocalVertexNormalizerTriangleIntersectionGeometryCache
        private

        def normalize_entity(entity)
          previous_stats = @local_vertex_normalizer_triangle_geometry_cache_stats
          previous_context = @local_vertex_normalizer_triangle_geometry_cache_context
          @local_vertex_normalizer_triangle_geometry_cache_stats = {
            collect_calls: 0,
            contexts: 0,
            normal_calls: 0,
            normal_hits: 0,
          ...
          @local_vertex_normalizer_triangle_geometry_cache_context = nil

          super
        ensure
          stats = @local_vertex_normalizer_triangle_geometry_cache_stats
          emit_triangle_intersection_geometry_cache(stats) if stats
          @local_vertex_normalizer_triangle_geometry_cache_stats = previous_stats
          @local_vertex_normalizer_triangle_geometry_cache_context = previous_context
        end
        ...
        def collect_triangle_intersection_failures(triangles, partition_ids: nil)
          ...
          if root_context
            @local_vertex_normalizer_triangle_geometry_cache_context = {
              normals: {}
            }
            stats[:contexts] += 1
          end
          ...
          if stats && root_context
            context = @local_vertex_normalizer_triangle_geometry_cache_context
        ...
        def emit_triangle_intersection_geometry_cache(stats)
      ...
      class LocalVertexNormalizer
        prepend LocalVertexNormalizerTriangleIntersectionGeometryCache unless
          ancestors.include?(LocalVertexNormalizerTriangleIntersectionGeometryCache)
      end
    end
  end
end
