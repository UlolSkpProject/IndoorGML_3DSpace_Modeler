...
module ULOL
  module Indoor3DGmlModeler
    module IndoorCore
      ...
      # The counters are active only while diagnostics/report profiling is enabled and
      # never alter geometry decisions or cached values used by production code.
      module LocalVertexNormalizerPerformanceProfiler
        private
        ...
        def normalize_entity(entity)
          ...
          previous_exact_context = @local_vertex_normalizer_exact_polygon_workload
          @local_vertex_normalizer_exact_polygon_workload = {
            calls: 0,
            total_seconds: 0.0,
            failures: 0,
            by_role: {},
          ...
          if profile
            emit_exact_polygon_workload_summary(
              profile,
              @local_vertex_normalizer_exact_polygon_workload
            )
            @local_vertex_normalizer_exact_polygon_workload = previous_exact_context
          end
        end
        ...
        def source_boundary_triangle_records(face, source_face_key)
          ...
          previous_face_key = @local_vertex_normalizer_exact_polygon_face_key
          @local_vertex_normalizer_exact_polygon_face_key = source_face_key
          super
        ensure
          if profile
            @local_vertex_normalizer_exact_polygon_face_key = previous_face_key
          end
        end

        def triangulate_exact_polygon_with_holes(outer, holes, drop_axis)
          profile = @local_vertex_normalizer_diagnostic_profile
          context = @local_vertex_normalizer_exact_polygon_workload
          return super unless profile && context

          context[:wrapper_calls] += 1
          role = @local_vertex_normalizer_diagnostic_snapshot_role || :unscoped
          face_key = @local_vertex_normalizer_exact_polygon_face_key
          ...
          signature = begin
            exact_polygon_workload_signature(outer, holes, drop_axis)
          ...
          if profile && context && started_at
            ...
            begin
              ...
              puts "[LVN DIAGNOSTIC] WORK exact_polygon_record_error #{error.class}: #{error.message}" if
                profile[:verbose]
            end
          end
        end
        ...
        def conforming_triangle_snapshot(
          ...
          previous_context = @local_vertex_normalizer_conforming_workload
          context = {
          ...
          @local_vertex_normalizer_conforming_workload = context
          ...
          if context
            record_conforming_workload(context)
            @local_vertex_normalizer_conforming_workload = previous_context
          end
        end
        ...
        def conforming_edge_interior_points(
          ...
          context = @local_vertex_normalizer_conforming_workload
          if context
        ...
        def exact_polygon_workload_signature(outer, holes, drop_axis)
          outer_signature = exact_polygon_loop_edge_signature(outer)
          hole_signatures = Array(holes).map do |hole|
            exact_polygon_loop_edge_signature(hole)
          end.sort
          "axis=#{drop_axis};outer=#{outer_signature};holes=#{hole_signatures.join('||')}"
        end

        def exact_polygon_loop_edge_signature(loop)
          points = Array(loop).map { |point| exact_polygon_point_signature(point) }
        ...
        def exact_polygon_point_signature(point)
        ...
        def record_exact_polygon_workload(
          workload,
          role:,
          face_key:,
          signature:,
        ...
        def emit_exact_polygon_workload_summary(profile, workload)
          return unless profile[:verbose]
          return unless respond_to?(:diagnostic_profile_log, true)

          unless workload
            diagnostic_profile_log('WORK exact_polygon_summary state=no_context')
            return
          end
          ...
          puts "[LVN DIAGNOSTIC] WORK exact_polygon_summary_error #{error.class}: #{error.message}"
        end

        def record_conforming_workload(context)
          profile = @local_vertex_normalizer_diagnostic_profile
          ...
          return unless profile[:verbose] && respond_to?(:diagnostic_profile_log, true)
      ...
      class LocalVertexNormalizer
        prepend LocalVertexNormalizerPerformanceProfiler unless
          ancestors.include?(LocalVertexNormalizerPerformanceProfiler)
      end
    end
  end
end
