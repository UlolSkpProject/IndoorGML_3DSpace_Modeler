...
module ULOL
  module Indoor3DGmlModeler
    module IndoorCore
      ...
      module LocalVertexNormalizerSourceBoundaryNormalization
        ...
        def triangle_snapshot(entities)
          ...
          @source_boundary_normalization_plan =
            source_boundary_normalization_plan(faces)
          records = super
          apply_source_boundary_constraint_aliases!(
            @source_boundary_normalization_plan
          )
          capture_source_boundary_snapshot_stats(faces, records)
          records
          ...
          @source_boundary_normalization_plan = nil
          records = super
          capture_source_boundary_snapshot_stats(faces, records, fallback: true)
          records
        ensure
          @source_boundary_normalization_plan = nil
        end
        ...
        def source_boundary_triangle_records(face, source_face_key)
          ...
          begin
            source_boundary_normalized_triangle_records(
              face,
              source_face_key,
              loop_entries
            )
        ...
        def source_boundary_normalization_plan(faces)
          loop_sets = faces.filter_map do |face|
            ...
            loops = face.loops.map.with_index do |loop, loop_index|
              ...
              simplified = simplify_source_boundary_loop(entries)
          ...
          targets = reject_conflicting_source_boundary_constraints(
            loop_sets,
            targets
          )
          occurrences.select! { |(_face, _loop, vertex_id), _| targets.key?(vertex_id) }
          targets = reject_unsafe_source_boundary_targets(faces, targets)
          occurrences.select! { |(_face, _loop, vertex_id), _| targets.key?(vertex_id) }
          constraint_aliases = source_boundary_constraint_aliases(
            loop_sets,
            targets
          )
        ...
        def reject_conflicting_source_boundary_constraints(loop_sets, targets)
          constraints = @source_boundary_axis_plane_plan &&
            @source_boundary_axis_plane_plan[:constraints]
        ...
        def source_boundary_constraint_aliases(loop_sets, targets)
          aliases = {}
          loop_sets.each do |face_entry|
            face_entry[:loops].each do |loop_entry|
        ...
        def apply_source_boundary_constraint_aliases!(plan)
          axis_plan = @source_boundary_axis_plane_plan
        ...
        def capture_source_boundary_snapshot_stats(
          faces,
          records,
          fallback: false
        )
          stats = (@source_boundary_normalization_stats || {}).dup
          ...
          @source_boundary_normalization_stats = stats
        end

        def simplify_source_boundary_loop(entries)
          ...
          loop do
            ...
            index = working.each_index.find do |candidate_index|
              ...
              source_boundary_redundant_point?(
                current,
                previous,
                following
              )
          ...
          entries.each_with_index do |entry, index|
            ...
            previous = source_boundary_surviving_neighbor(
              entries,
              survivor_ids,
              index,
              -1
            )
            following = source_boundary_surviving_neighbor(
              entries,
              survivor_ids,
              index,
              1
            )
            projection = source_boundary_projection(
              entry[:point],
              previous[:point],
              following[:point]
            )
        ...
        def source_boundary_surviving_neighbor(
          entries,
          survivor_ids,
          start_index,
          direction
        ...
        def source_boundary_redundant_point?(point, endpoint_a, endpoint_c)
          projection = source_boundary_projection(point, endpoint_a, endpoint_c)
        ...
        def source_boundary_projection(point, endpoint_a, endpoint_c)
          direction = [
        ...
        def reject_unsafe_source_boundary_targets(faces, targets)
        ...
        def source_boundary_normalized_loop_entries(face, plan)
          ...
          face.loops.map.with_index do |loop, loop_index|
            ...
            compact_source_boundary_entries(entries)
          end
        end

        def compact_source_boundary_entries(entries)
          compact = []
        ...
        def source_boundary_normalized_triangle_records(
          face,
          source_face_key,
          loop_entries
        )
          ...
          records = subdivide_source_boundary_records(
            records,
            topology_points
          )
        ...
        def subdivide_source_boundary_records(records, candidates)
          signatures = {}
          edge_split_cache = {}
      ...
      class LocalVertexNormalizer
        prepend LocalVertexNormalizerSourceBoundaryNormalization unless
          ancestors.include?(LocalVertexNormalizerSourceBoundaryNormalization)
      end
    end
  end
end
