...
module ULOL
  module Indoor3DGmlModeler
    module IndoorCore
      ...
      module LocalVertexNormalizerComplexFaceSkip
        def normalize(
          entity,
          ...
          **options
          ...
          if commit_on_failure
            raise ArgumentError,
                  'commit_on_failure is disabled for LocalVertexNormalizer'
          end

          started_at = Process.clock_gettime(Process::CLOCK_MONOTONIC)
          skip_report = begin
            complex_face_skip_report(entity)
          ...
          attach_complex_face_skip_profile(
            entity,
            skip_report,
            elapsed,
            diagnostics: diagnostics,
        ...
        def complex_face_skip_report(entity)
          entities = entity.definition.entities
          inventory = complex_face_inventory(entities)
        ...
        def complex_face_inventory(entities)
          ...
          faces.each_with_index do |face, face_index|
            record = complex_face_record(face, face_index)
            next unless record
        ...
        def complex_face_record(face, face_index)
          ...
          loop_counts = ordered_loops.map do |loop|
            ...
            keys = vertices.map do |vertex|
              complex_face_grid_key(vertex.position)
            end
            complex_face_compact_loop(keys).length
          end
          ...
          bridge_candidates = complex_face_bridge_work(
            outer_count,
            hole_counts
          )
        ...
        def complex_face_bridge_work(outer_count, hole_counts)
        ...
        def attach_complex_face_skip_profile(
          entity,
          skip_report,
          elapsed,
          diagnostics:,
          ...
          geometry = diagnostic_geometry_counts(entity)
          profile = {
            enabled: true,
          ...
          if profile[:verbose]
            puts format(
              '[LVN DIAGNOSTIC] PROFILE SKIPPED total=%.6fs reason=%s complex_faces=%d',
              elapsed,
          ...
          skip_report[:diagnostic_profile] = profile
        end
      end
      ...
      class LocalVertexNormalizer
        ...
        prepend LocalVertexNormalizerComplexFaceSkip unless
          ancestors.include?(LocalVertexNormalizerComplexFaceSkip)
      end
    end
  end
end
