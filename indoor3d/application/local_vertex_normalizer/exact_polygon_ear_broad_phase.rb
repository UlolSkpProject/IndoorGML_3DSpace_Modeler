...
module ULOL
  module Indoor3DGmlModeler
    module IndoorCore
      ...
      module LocalVertexNormalizerExactPolygonEarBroadPhase
        EXACT_POLYGON_EAR_BROAD_PHASE_THRESHOLD = 24

        private

        def normalize_entity(entity)
          previous_stats = @local_vertex_normalizer_exact_ear_broad_phase_stats
          stats = {
            triangulations: 0,
          ...
          @local_vertex_normalizer_exact_ear_broad_phase_stats = stats

          super
        ensure
          if stats
            emit_exact_polygon_ear_broad_phase(stats)
            @local_vertex_normalizer_exact_ear_broad_phase_stats = previous_stats
          end
        end
        ...
        def triangulate_exact_weak_polygon(points, drop_axis)
          ...
          previous_context = @local_vertex_normalizer_exact_ear_broad_phase_context
          context = {
            projected: nil,
            length: nil,
            index: nil
          }
          @local_vertex_normalizer_exact_ear_broad_phase_context = context
          stats[:triangulations] += 1

          super
        ensure
          if stats
            @local_vertex_normalizer_exact_ear_broad_phase_context = previous_context
          end
        end
        ...
        def exact_polygon_ear?(polygon, index, drop_axis, projected: nil)
          ...
          stats[:ear_calls] += 1
          broad_phase = exact_polygon_ear_broad_phase_index(projected, context, stats)
          ...
          triangle_bounds = exact_polygon_ear_bounds_2d([point_a, point_b, point_c])
          ...
          diagonal_bounds = exact_polygon_ear_bounds_2d([point_a, point_c])
          ...
          exact_polygon_ear_broad_phase_fallback(stats, error)
          super
        end

        def exact_polygon_ear_broad_phase_index(projected, context, stats)
        ...
        def exact_polygon_ear_point_candidates(index, bounds, stats)
          point_axes = index.fetch(:point_axes)
          ranges = 2.times.map do |axis|
            entries = point_axes[axis]
            first = exact_polygon_ear_lower_bound(entries, bounds[axis][0])
            last = exact_polygon_ear_upper_bound(entries, bounds[axis][1])
        ...
        def exact_polygon_ear_edge_candidates(index, bounds, stats)
          edge_count = index.fetch(:edge_bounds).length
          options = []

          2.times do |axis|
            min_entries = index.fetch(:edge_min_axes)[axis]
            prefix_end = exact_polygon_ear_upper_bound(min_entries, bounds[axis][1])
            options << [:prefix, axis, 0, prefix_end, prefix_end]

            max_entries = index.fetch(:edge_max_axes)[axis]
            suffix_start = exact_polygon_ear_lower_bound(max_entries, bounds[axis][0])
        ...
        def exact_polygon_ear_upper_bound(entries, value)
          low = 0
          high = entries.length
          while low < high
            middle = (low + high) / 2
        ...
        def exact_polygon_ear_broad_phase_fallback(stats, error)
          return unless stats
        ...
        def emit_exact_polygon_ear_broad_phase(stats)
      ...
      class LocalVertexNormalizer
        prepend LocalVertexNormalizerExactPolygonEarBroadPhase unless
          ancestors.include?(LocalVertexNormalizerExactPolygonEarBroadPhase)
      end
    end
  end
end
