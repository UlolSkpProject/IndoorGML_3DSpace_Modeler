...
module ULOL
  module Indoor3DGmlModeler
    module IndoorCore
      ...
      module LocalVertexNormalizerSurfaceDescriptorSegmentBroadPhase
        SURFACE_DESCRIPTOR_SEGMENT_BROAD_PHASE_THRESHOLD = 32

        private

        def normalize_entity(entity)
          previous_stats = @local_vertex_normalizer_surface_descriptor_segment_bp_stats
          previous_context = @local_vertex_normalizer_surface_descriptor_segment_bp_context
          @local_vertex_normalizer_surface_descriptor_segment_bp_stats = {
            owner_calls: 0,
          ...
          @local_vertex_normalizer_surface_descriptor_segment_bp_context = nil

          super
        ensure
          stats = @local_vertex_normalizer_surface_descriptor_segment_bp_stats
          emit_surface_descriptor_segment_broad_phase(stats) if stats
          @local_vertex_normalizer_surface_descriptor_segment_bp_stats = previous_stats
          @local_vertex_normalizer_surface_descriptor_segment_bp_context = previous_context
        end
        ...
        def split_triangle_edge_owners(records)
          ...
          stats[:owner_calls] += 1
          previous_context = @local_vertex_normalizer_surface_descriptor_segment_bp_context
          @local_vertex_normalizer_surface_descriptor_segment_bp_context = {
            candidates: nil,
            axes: nil,
            indexed: false
          }
          ...
          @local_vertex_normalizer_surface_descriptor_segment_bp_context = previous_context if stats
        end

        def integer_points_on_segment_sorted(point_a, point_b, candidates)
          stats = @local_vertex_normalizer_surface_descriptor_segment_bp_stats
          context = @local_vertex_normalizer_surface_descriptor_segment_bp_context
          ...
          narrowed = surface_descriptor_segment_candidates(
            point_a,
            point_b,
            candidates,
            context,
          ...
          surface_descriptor_segment_broad_phase_fallback(stats, error)
          super
        end

        def surface_descriptor_segment_candidates(
          point_a,
          point_b,
          candidates,
          context,
          ...
          index = surface_descriptor_segment_index(candidates, context, stats)
          ...
          ranges = 3.times.map do |axis|
            entries = index[axis]
            first = surface_descriptor_segment_lower_bound(entries, bounds[axis][0])
            last = surface_descriptor_segment_upper_bound(entries, bounds[axis][1])
        ...
        def surface_descriptor_segment_index(candidates, context, stats)
        ...
        def surface_descriptor_segment_lower_bound(entries, value)
          low = 0
          high = entries.length
          while low < high
            middle = (low + high) / 2
        ...
        def surface_descriptor_segment_upper_bound(entries, value)
          low = 0
          high = entries.length
          while low < high
            middle = (low + high) / 2
        ...
        def surface_descriptor_segment_broad_phase_fallback(stats, error)
          return unless stats
        ...
        def emit_surface_descriptor_segment_broad_phase(stats)
      ...
      class LocalVertexNormalizer
        prepend LocalVertexNormalizerSurfaceDescriptorSegmentBroadPhase unless
          ancestors.include?(LocalVertexNormalizerSurfaceDescriptorSegmentBroadPhase)
      end
    end
  end
end
