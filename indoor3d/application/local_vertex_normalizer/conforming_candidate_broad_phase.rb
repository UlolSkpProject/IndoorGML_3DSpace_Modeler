...
module ULOL
  module Indoor3DGmlModeler
    module IndoorCore
      ...
      module LocalVertexNormalizerConformingCandidateBroadPhase
        private
        ...
        def conforming_triangle_snapshot(
          ...
          previous_context = @local_vertex_normalizer_conforming_broad_phase
          context = {
          ...
          @local_vertex_normalizer_conforming_broad_phase = context

          super
        ensure
          if context
            emit_conforming_broad_phase(context)
            @local_vertex_normalizer_conforming_broad_phase = previous_context
          end
        end
        ...
        def conforming_edge_interior_points_in_direction(
          ...
          context = @local_vertex_normalizer_conforming_broad_phase
          return super unless context

          narrowed = conforming_broad_phase_candidates(
            start_point,
            end_point,
            candidates,
            context,
        ...
        def conforming_broad_phase_candidates(
          start_point,
          end_point,
          candidates,
          context,
          ...
          index = conforming_candidate_index(candidates, context, coordinate_space)
          unless index
            context[:fallbacks] += 1
          ...
          ranges = 3.times.map do |axis|
            entries = index[axis]
            first = conforming_axis_lower_bound(entries, bounds[axis][0])
            last = conforming_axis_upper_bound(entries, bounds[axis][1])
        ...
        def conforming_candidate_index(candidates, context, coordinate_space)
        ...
        def conforming_axis_lower_bound(entries, value)
          low = 0
          high = entries.length
          while low < high
            middle = (low + high) / 2
        ...
        def conforming_axis_upper_bound(entries, value)
          low = 0
          high = entries.length
          while low < high
            middle = (low + high) / 2
        ...
        def emit_conforming_broad_phase(context)
      ...
      class LocalVertexNormalizer
        prepend LocalVertexNormalizerConformingCandidateBroadPhase unless
          ancestors.include?(LocalVertexNormalizerConformingCandidateBroadPhase)
      end
    end
  end
end
