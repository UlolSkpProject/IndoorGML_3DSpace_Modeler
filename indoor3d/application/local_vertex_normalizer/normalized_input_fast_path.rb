  ...
  module Indoor3DGmlModeler
    module IndoorCore
      class LocalVertexNormalizer
        ...
        unless private_method_defined?(:normalize_entity_before_normalized_input_fast_path)
          alias_method :normalize_entity_before_normalized_input_fast_path,
                       :normalize_entity
        end
        ...
        # final_coplanar_face_merge wrapper still runs unchanged after this
        # method returns.
        def normalize_entity(entity)
          return normalize_entity_before_normalized_input_fast_path(entity) unless
            normalized_input_fast_path_candidate?(entity)

          ensure_unique_definition(entity)
          result = try_normalized_input_fast_path(entity)
          return result if result

          normalize_entity_before_normalized_input_fast_path(entity)
        end
        ...
        def try_normalized_input_fast_path(entity)
          ...
          return normalized_input_fast_path_fallback(:grid_residual) if
          ...
          return normalized_input_fast_path_fallback(:triangle_cleanup_required) if
            normalized_input_fast_path_cleanup_required?(cleanup)
          return normalized_input_fast_path_fallback(:duplicate_triangle_cleanup_required) if
          ...
          return normalized_input_fast_path_fallback(:topology_changed_during_probe) unless
            current_topology == topology
          return normalized_input_fast_path_fallback(:not_closed_after_probe) unless
            closed_topology?(current_topology)
          return normalized_input_fast_path_fallback(:not_manifold_after_probe) unless
          ...
          report = build_normalized_input_fast_path_report(
            entity,
          ...
          emit_normalized_input_fast_path_work(
            applied: true,
          ...
          normalized_input_fast_path_fallback(
        ...
        def normalized_input_fast_path_cleanup_required?(cleanup)
        ...
        def normalized_input_fast_path_fallback(reason, error: nil)
          @normalized_input_fast_path_baseline = nil
          emit_normalized_input_fast_path_work(
            applied: false,
            reason: reason,
            error: error
        ...
        def build_normalized_input_fast_path_report(
          entity,
          topology:,
          vertices:,
          triangles:,
          ...
          report[:normalization_strategy] = :already_normalized_fast_path
        ...
        def emit_normalized_input_fast_path_work(
          applied:,
          reason:,
          triangle_count: nil,
          error: nil
        )
          return unless @local_vertex_normalizer_diagnostic_profile
          ...
          puts "[LVN DIAGNOSTIC] WORK normalized_input_fast_path #{fields.join(' ')}"
        rescue StandardError
          nil
        end
      end
