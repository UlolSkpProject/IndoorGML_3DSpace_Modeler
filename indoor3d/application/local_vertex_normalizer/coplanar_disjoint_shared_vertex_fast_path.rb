...
module ULOL
  module Indoor3DGmlModeler
    module IndoorCore
      ...
      module LocalVertexNormalizerCoplanarDisjointSharedVertexFastPath
        private

        def normalize_entity(entity)
          previous_stats =
            @local_vertex_normalizer_coplanar_simplex_fast_path_stats
          @local_vertex_normalizer_coplanar_simplex_fast_path_stats = {
            coplanar_calls: 0,
            disjoint_calls: 0,
          ...
          stats = @local_vertex_normalizer_coplanar_simplex_fast_path_stats
          emit_coplanar_simplex_fast_path(stats) if stats
          @local_vertex_normalizer_coplanar_simplex_fast_path_stats =
            previous_stats
        end
        ...
        def coplanar_triangle_intersection_allowed?(
          ...
          when 0
            ...
            decision = coplanar_disjoint_intersection_decision(
              triangle_a,
              triangle_b
            )
          ...
          when 1
            ...
            decision = coplanar_shared_vertex_intersection_decision(
              triangle_a,
              triangle_b,
              shared.first
            )
          ...
          coplanar_simplex_fast_path_fallback(stats, error)
          super
        end
        ...
        def coplanar_disjoint_intersection_decision(triangle_a, triangle_b)
          projected = coplanar_simplex_projection(triangle_a, triangle_b)
          return nil unless projected

          projected_a, projected_b = projected
          return true if coplanar_strict_edge_separation?(
            projected_a,
            projected_b
          )
        ...
        def coplanar_shared_vertex_intersection_decision(
          triangle_a,
          triangle_b,
          shared_vertex
        )
          ...
          edges_a = coplanar_triangle_edges_2d(projected_a)
          edges_b = coplanar_triangle_edges_2d(projected_b)
          edges_a.each do |edge_a|
            edges_b.each do |edge_b|
              ...
              return nil unless coplanar_edges_contact_only_at_shared_vertex?(
                edge_a,
                edge_b,
                shared
              )
        ...
        def coplanar_simplex_projection(triangle_a, triangle_b)
        ...
        def coplanar_strict_edge_separation?(triangle_a, triangle_b)
          [
        ...
        def coplanar_triangle_edges_2d(triangle)
          3.times.map do |index|
            [triangle[index], triangle[(index + 1) % 3]]
          end
        end

        def coplanar_edges_contact_only_at_shared_vertex?(
          edge_a,
          edge_b,
          shared
        )
        ...
        def coplanar_simplex_fast_path_fallback(stats, error)
          return unless stats
        ...
        def emit_coplanar_simplex_fast_path(stats)
      ...
      class LocalVertexNormalizer
        prepend LocalVertexNormalizerCoplanarDisjointSharedVertexFastPath unless
          ancestors.include?(
            LocalVertexNormalizerCoplanarDisjointSharedVertexFastPath
          )
      end
    end
  end
