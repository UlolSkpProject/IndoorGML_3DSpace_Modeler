...
module ULOL
  module Indoor3DGmlModeler
    module IndoorCore
      ...
      # O(1) feature-id lookup layered over the existing FeatureRegistry API.
      # Snapshot/restore rebuild the index from the authoritative feature arrays,
      # so old snapshots remain compatible.
      module FeatureRegistryIdIndex
        def reset!
          result = super
          @features_by_id = {}
          result
        ...
        def snapshot
          super.merge(features_by_id: Hash(@features_by_id).dup)
        end
      ...
      FeatureRegistry.prepend(FeatureRegistryIdIndex) unless FeatureRegistry.ancestors.include?(FeatureRegistryIdIndex)
