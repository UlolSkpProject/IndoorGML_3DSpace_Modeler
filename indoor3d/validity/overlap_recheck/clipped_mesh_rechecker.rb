...
require_relative 'clipped_operand_analyzer'
    ...
    module IndoorCore
      module IndoorGmlConverter
        module Val3dityClippedMeshRecheck
          MODE = 'clipped_mesh_proxy_then_target_boolean'
          GUARD_MARGIN_MM = 5.0
          LEAF_TRIANGLE_COUNT = 12

          class RebuildAnalyzer < ClippedOperandAnalyzer
            PLANES = ClippedOperandAnalyzer::PLANES
            ...
            def cached_rebuild_mesh(group, cell_id, cache)
              key = [:clipped_mesh, group.object_id, cell_id.to_s]
            ...
            def rebuild_geometry(mesh, crop)
              ...
              counts[:triangle_bounds_rejected] =
                mesh[:triangles].length - candidate_indices.length
              ...
              return rebuild_error(report, 'surface_miss') if
                report['surface_miss_requires_containment_test']
              return rebuild_error(report, 'global_cap_open') unless
                global['closed_graph'] == true
              ...
              return rebuild_error(report, 'reconstructed_mesh_empty') if
                raw_triangles.empty?
              ...
              report['reconstructed_surface_triangle_count'] =
                clipped_triangles.length
              ...
              rebuild_error(
                count_report(
                  counts || {},
                  { 'closed_graph' => false, 'loop_count' => 0 }
                ),
                "#{e.class}: #{e.message}"
              )
            end
            ...
            def count_report(counts, global)
              {
                'candidate_triangle_count' =>
                  counts[:candidate_triangle_count].to_i,
                'triangle_bounds_rejected' =>
                  counts[:triangle_bounds_rejected].to_i,
                ...
                'fully_inside_triangle_count' =>
                  counts[:fully_inside_triangle_count].to_i,
            ...
            def degenerate_triangle?(triangle)
              ...
              ab = subtract(b, a)
              ac = subtract(c, a)
              magnitude_squared(cross(ab, ac)) <=
                [@weld * @weld * 1.0e-6, 1.0e-24].max
            end
            ...
            def canonical_plane_segments(plane_segments)
              ...
              counts.each do |(plane, a, b), count|
                ...
                retained[plane] << [
                  representatives.fetch(a), representatives.fetch(b)
                ]
              end
              retained
            end
            ...
            def build_cap_triangles(plane_segments, crop, mesh)
              ...
              PLANES.each do |plane, _axis, _side|
                result = cap_plane(
                  plane, plane_segments.fetch(plane), crop, mesh
                )
...

              {
                ...
                report: {
                  ...
                  'cap_region_count' => plane_reports.values.sum do |row|
                    row['inside_region_count'].to_i
                  end,
            ...
            def cap_plane(plane, segments, crop, mesh)
              ...
              unless segment_components_touch_boundary?(
                segment_edges, vertices, rectangle
              )
                return cap_plane_error(
                  segments, 'interior_plane_loop_requires_hole_triangulation'
                )
              end
              ...
              positive_cycles = cycles.select do |cycle|
                polygon_area_2d(cycle.map { |index| vertices[index] }) >
                  area_epsilon
              end
              ...
              positive_cycles.each do |cycle|
                polygon = simplify_polygon(
                  cycle.map { |index| vertices[index] }
                )
                next if polygon.length < 3

                ears = triangulate_polygon(polygon)
                return cap_plane_error(
                  segments, 'cap_ear_clipping_failed'
                ) if ears.nil?
                next if ears.empty?
                ...
                return cap_plane_error(
                  segments, 'cap_region_classification_ambiguous'
                ) if inside.nil?
                next unless inside
            ...
            def plane_rectangle(plane, crop)
              case plane
              when :min_x, :max_x
                [[crop[:min][1], crop[:min][2]],
                 [crop[:max][1], crop[:min][2]],
                 [crop[:max][1], crop[:max][2]],
                 [crop[:min][1], crop[:max][2]]]
              when :min_y, :max_y
                [[crop[:min][0], crop[:min][2]],
                 [crop[:max][0], crop[:min][2]],
                 [crop[:max][0], crop[:max][2]],
                 [crop[:min][0], crop[:max][2]]]
              else
                [[crop[:min][0], crop[:min][1]],
                 [crop[:max][0], crop[:min][1]],
                 [crop[:max][0], crop[:max][1]],
                 [crop[:min][0], crop[:max][1]]]
              end
            end
            ...
            def intern_2d(vertices, point)
              ...
              if existing &&
                 squared_distance_2d(vertices[existing], point) <= @weld * @weld
                existing
              else
            ...
            def rectangle_boundary_edges(vertices, rectangle)
              ...
              sides = [
                vertices.each_index.select do |i|
                  near?(vertices[i][1], min_v)
                end.sort_by { |i| vertices[i][0] },
                vertices.each_index.select do |i|
                  near?(vertices[i][0], max_u)
                end.sort_by { |i| vertices[i][1] },
                vertices.each_index.select do |i|
                  near?(vertices[i][1], max_v)
                end.sort_by { |i| -vertices[i][0] },
                vertices.each_index.select do |i|
                  near?(vertices[i][0], min_u)
                end.sort_by { |i| -vertices[i][1] }
              ]
              ...
              edges2d.each do |a, b|
                [[a, b], [b, a]].each do |start|
                  ...
                  loop do
                    ...
                    next_vertex = neighbors[
                      (incoming_index - 1) % neighbors.length
                    ]
            ...
            def simplify_polygon(points)
              ...
              while changed && result.length > 3
                ...
                result.length.times do |index|
                  ...
                  next unless cross_2d(
                    subtract_2d(b, a), subtract_2d(c, b)
                  ).abs <= area_epsilon
            ...
            def triangulate_polygon(points)
              ...
              while indices.length > 3
                ...
                indices.length.times do |position|
                  ...
                  next if indices.any? do |candidate|
                    next false if [prev, curr, nxt].include?(candidate)
              ...
              triangles << indices.map { |index| polygon[index] } if
                indices.length == 3
              triangles
            end
            ...
            def orient_cap_triangle(plane, triangle)
              ...
              dot(normal, desired).negative? ?
                [triangle[0], triangle[2], triangle[1]] : triangle
            end
            ...
            def ray_parity(origin, direction, mesh)
              ...
              hits = indices.filter_map do |index|
                ray_triangle_t(
                  origin, direction, mesh[:triangles].fetch(index)
                )
              ...
              hits.each do |value|
                if unique.empty? || (value - unique[-1]).abs > tolerance
                  unique << value
                end
              end
              unique.length.odd?
            ...
            def point_on_segment_parameter(point, a, b)
              ...
              epsilon = [
                @weld / Math.sqrt(length_squared), 1.0e-12
              ].max
              return nil if parameter < -epsilon || parameter > 1.0 + epsilon

              clamped = [[parameter, 0.0].max, 1.0].min
              projected = 3.times.map do |axis|
                a[axis] + ab[axis] * clamped
              end
              return nil if squared_distance(point, projected) > @weld * @weld
            ...
            def deduplicate_cycle_points(points)
              ...
              points.each do |point|
                next if result.any? &&
                        squared_distance(result[-1], point) <= @weld * @weld
            ...
            def triangle_soup_topology(triangles)
              ...
              non_manifold_count = edge_counts.count do |_edge, count|
                count > 2
              end
              {
                ...
                'closed_two_manifold' =>
                  boundary_count.zero? && non_manifold_count.zero? &&
            ...
            def build_bvh(bounds, indices)
              ...
              extents = 3.times.map do |axis|
                node_bounds[:max][axis] - node_bounds[:min][axis]
              end
              axis = extents.each_with_index.max_by do |value, _index|
                value
              end[1]
            ...
            def union_bounds(bounds)
              {
                min: 3.times.map do |axis|
                  bounds.map { |row| row[:min][axis] }.min
                end,
                max: 3.times.map do |axis|
                  bounds.map { |row| row[:max][axis] }.max
                end
              }
            end
            ...
            def query_box(node, box, output = [])
              ...
              return output unless bounds_overlap?(
                { min: node[:min], max: node[:max] }, box
              )
            ...
            def query_ray(node, origin, direction, output = [])
              return output unless node
              return output unless ray_hits_bounds?(
                origin, direction, node[:min], node[:max]
              )
            ...
            def ray_hits_bounds?(origin, direction, min, max)
              ...
              3.times do |axis|
                if direction[axis].abs <= 1.0e-15
                  return false if
                    origin[axis] < min[axis] - @weld ||
                    origin[axis] > max[axis] + @weld
                  next
                end
          ...
          class Rechecker < Val3dityFullIntersectionRechecker
            ...
            def model_solid_intersection_for_pair(
              ...
              analyzed = @rebuild_analyzer.analyze_rebuild(
                group1, group2, cells, @mesh_cache
              )
              ...
              if report['status'] != 'ok' || analyzed[:geometry].nil?
                return full_confirmation(
                  ...
                  cell_id1,
                  cell_id2,
                  report['error'] || 'rebuild_analysis_failed',
                  started
                )
              end
              ...
              if proxy_result[:fallback]
                return full_confirmation(
                  ...
                  cell_id1,
                  cell_id2,
                  proxy_result[:fallback_reason] || 'direct_proxy_failed',
                  started
                )
              end

              record['path'] = 'clipped_mesh_recheck'
              record['intersection_status'] = proxy_result[:status].to_s
              record['intersection_reason'] = proxy_result[:reason].to_s if
                proxy_result[:reason]
              record['intersection_volume_in3'] = proxy_result[:volume] if
                proxy_result.key?(:volume)
              record['intersection_component_count'] =
                proxy_result[:component_count] if
              ...
              record ||= {
                'cells' => cells || [cell_id1.to_s, cell_id2.to_s],
                'mode' => MODE
              }
            ...
            def direct_proxy_intersection(
              source, target, cell_ids, geometry, record
            )
              ...
              @indoor_model.with_indoor_model_operation(
                'IndoorGML clipped mesh overlap recheck', rollback: true
              ) do
                proxy_started = clock
                proxy = build_proxy_group(
                  source, geometry.fetch(:triangles)
                )
                ...
                record['proxy_surface_triangle_count'] =
                  geometry[:clipped_surface_triangle_count]
                record['proxy_cap_triangle_count'] = geometry[:cap_triangle_count]
                next fallback_result('proxy_not_manifold') unless
                  valid_manifold_group?(proxy)

                proxy_volume = solid_group_volume(proxy)
                next fallback_result('proxy_nonpositive_volume') unless
                  proxy_volume&.positive?
              ...
              record['proxy_operation_exception'] =
                "#{e.class}: #{e.message}"
            ...
            def build_proxy_group(source, triangles)
              ...
              group.name = '__IndoorGML_CLIPPED_MESH_PROXY__' if
            ...
            def classify_proxy_result(result, cell_ids)
              ...
              return non_solid_intersection_result(
                result, faces, edges
              ) unless valid_manifold_group?(result)

              volume = solid_group_volume(result)
              return non_solid_intersection_result(
                result, faces, edges
              ) unless volume&.positive?
            ...
            def full_confirmation(
              ...
              record['path'] = 'full_geometry_confirmation'
              record['fallback_reason'] = reason.to_s
              record['fallback_elapsed_ms'] = elapsed_ms(confirmation_started)
              record['intersection_status'] = result[:status].to_s if
                result.is_a?(Hash)
            ...
            def valid_faces(group)
              return [] unless
                group&.valid? && group.respond_to?(:definition)
