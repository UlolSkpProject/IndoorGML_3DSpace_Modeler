...
> SketchUp 2026용 IndoorGML 1.0 모델링 및 Export Ruby Extension
...
- [사용 방법](#사용-방법)
- [기능 목록](#기능-목록)
- [IndoorGML 1.0 구현 범위](#indoorgml-10-구현-범위)
...
| ![](indoor3d/assets/icons/export_gml.png)              | Export GML               | IndoorGML 1.0 형식으로 `.gml` 파일 저장 (유효성 검사 없음) |
...
현재 Exporter는 `3D CellSpace + 단일 SpaceLayer + State/Transition dual graph` 범위를 대상으로 IndoorGML 1.0 Core/Navigation XML을 생성합니다.
...
레거시의 `indoorCreator.exe` 대신, Ruby로 직접 IndoorGML 1.0 스펙에 맞는 `.gml` 파일을 생성함.
val3dity 및 InViewer 테스트에서 문제 없음을 확인함.

Export 좌표는 현재 SketchUp Ruby API의 내부 좌표값을 사용합니다. SketchUp 내부 길이 단위는 inch이며, 현재 GML에는 별도 `srsName`/CRS 또는 단위 변환 정책을 기록하지 않습니다.
