        ...
        module Val3dityRecheckV3MeshProxy
          class RebuildAnalyzer
            unless private_method_defined?(:cap_plane_without_interior_loop_decomposition)
              alias_method :cap_plane_without_interior_loop_decomposition, :cap_plane
            end

            private

            # The existing planar-face path is retained for all geometry it
            # already handles. Only a disconnected interior cap loop enters this
            # decomposition path. Each vertical slab is bounded by the retained
            # cap segments, classified against the source mesh, and triangulated.
            # Existing conforming-triangle and closed-two-manifold hard gates
            # remain the final authority before a SketchUp proxy is constructed.
            def cap_plane(plane, segments, crop, mesh)
              result = cap_plane_without_interior_loop_decomposition(
                plane, segments, crop, mesh
              )
              return result unless
                result[:error].to_s == 'interior_plane_loop_requires_hole_triangulation'
            ...
            def cap_plane_by_vertical_decomposition(plane, segments, crop, mesh)
              ...
              u_breaks.each_cons(2) do |u0, u1|
                ...
                boundaries.each_cons(2) do |lower, upper|
                  ...
                  sample2 = [u_mid, (lower[:v_mid] + upper[:v_mid]) * 0.5]
...

              {
                ...
                report: {
                  ...
                  'skipped_narrow_slab_count' => skipped_narrow_slab_count,
            ...
            def cap_u_breaks(rows, min_u, max_u)
              ...
              values.each do |value|
                if clusters.empty? || (value - clusters[-1][-1]).abs > @weld
                  clusters << [value]
                else
                  clusters[-1] << value
                end
            ...
            def slab_boundaries(rows, u0, u1, u_mid, min_v, max_v)
              ...
              rows.each_with_index do |row, index|
                ...
                next unless u_mid > min_row_u + @weld * 0.1 &&
              ...
              boundaries.sort_by! do |boundary|
                [boundary[:v_mid], boundary[:kind] == :segment ? 0 : 1,
                 boundary[:index].to_i]
              end
            ...
            def deduplicate_polygon_2d(points)
              ...
              points.each do |point|
                next if result.any? &&
                        squared_distance_2d(result[-1], point) <= @weld * @weld
            ...
            def triangle_area_2d_abs(triangle)
              a, b, c = triangle
              (cross_2d(subtract_2d(b, a), subtract_2d(c, a)) * 0.5).abs
            end
