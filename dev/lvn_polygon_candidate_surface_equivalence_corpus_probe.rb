...
# Load the single-solid Phase 4 probe without triggering its default auto-run.
...
# Phase 4 representative-corpus surface-equivalence probe.
#
# Reuses the single-solid Phase 4 implementation unchanged. The runner selects
# deterministic no-hole face-count quantiles plus every eligible inner-loop
# source, invokes the production target-surface/equivalence path one source at a
# time, captures diagnostics, and restores the original selection.
module ULOL
  module Indoor3DGmlModeler
    module IndoorCore
      module LvnPolygonCandidateSurfaceEquivalenceCorpusProbe
        ...
        def run(
          ...
          if selected.empty?
            puts '[LVN POLYGON SURFACE PHASE 4 CORPUS] Select solids first.'
            return false
          end

          puts '=' * 108
          puts ' LVN Polygon-Preserving Candidate — Phase 4 Representative Surface-Equivalence Corpus'
          puts '=' * 108
          ...
          targets.each_with_index do |target, index|
            ...
            result = target.merge(
              ...
              expected_elapsed_ms: extract_metric_ms(captured, 'expected elapsed'),
              candidate_gate_elapsed_ms:
                extract_metric_ms(captured, 'candidate gate elapsed'),
              equivalence_elapsed_ms:
                extract_metric_ms(captured, 'equivalence elapsed'),
              phase4_elapsed_ms: extract_metric_ms(captured, 'Phase 4 elapsed'),
              passed: passed == true,
              output: captured
            )
            results << result
            ...
            expected_text = metric_text(result[:expected_elapsed_ms])
            gate_text = metric_text(result[:candidate_gate_elapsed_ms])
            equivalence_text = metric_text(result[:equivalence_elapsed_ms])
            phase4_text = metric_text(result[:phase4_elapsed_ms])
            puts format(
              '[%02d/%02d] %-50s type=%-5s F=%4d inner=%3d exp=%s gate=%s eq=%s p4=%s ms total=%9.3f ms %s',
              index + 1,
              targets.length,
              result[:label][0, 50],
              category,
            ...
            puts '  ---- captured surface-equivalence tail ----'
          ...
          expected_times = results.filter_map { |result| result[:expected_elapsed_ms] }
          gate_times = results.filter_map { |result| result[:candidate_gate_elapsed_ms] }
          equivalence_times = results.filter_map { |result| result[:equivalence_elapsed_ms] }
          phase4_times = results.filter_map { |result| result[:phase4_elapsed_ms] }

          selection_restored = restore_selection(model, original_selection)

          puts '=' * 108
          puts ' Phase 4 Representative Surface-Equivalence Summary'
          puts '=' * 108
          ...
          puts format('surface PASS          : %d', passed_results.length)
          puts format('surface FAIL          : %d', failed_results.length)
          puts format('total elapsed         : %.3f s', total_ms / 1000.0)
          puts format('median total elapsed  : %.3f ms', median(results.map { |r| r[:elapsed_ms] }))
          print_time_summary('expected target', expected_times)
          print_time_summary('candidate gate', gate_times)
          print_time_summary('equivalence', equivalence_times)
          print_time_summary('Phase 4', phase4_times)
          ...
          unless failed_results.empty?
            puts '-' * 108
            puts 'Failed surface-equivalence samples:'
          ...
          warn "[LVN POLYGON SURFACE PHASE 4 CORPUS] #{error.class}: #{error.message}"
        ...
        def metric_text(value)
          value ? format('%8.3f', value) : '     n/a'
        end

        def print_time_summary(label, values)
          return if values.empty?

          puts format('median %-14s: %.3f ms', label, median(values))
          puts format('maximum %-13s: %.3f ms', label, values.max)
        end
...
ULOL::Indoor3DGmlModeler::IndoorCore::
  LvnPolygonCandidateSurfaceEquivalenceCorpusProbe.run
