...
module ULOL
  module Indoor3DGmlModeler
    module IndoorCore
      class LocalGridRuntimeRefreshPolicyV2Test < Minitest::Test
        ...
        def test_initial_model_load_dispatches_to_hard_refresh
          ...
          model.define_singleton_method(:hard_refresh_runtime_data_local_grid_v2) do |initial_model_load: false|
            calls << [:hard, initial_model_load]
            :hard
          end
          model.define_singleton_method(:soft_refresh_runtime_data_local_grid_v2) do
            calls << :soft
            :soft
          end
        ...
        def test_v2_generic_entrypoint_is_soft_unless_initial_load
          model, calls = build_dispatch_model(enabled: false)
          model.define_singleton_method(:hard_refresh_runtime_data_local_grid_v2) do |initial_model_load: false|
            calls << [:hard, initial_model_load]
            :hard
          ...
          result = IndoorModel.instance_method(:refresh_runtime_data_local_grid_v2).bind(model).call
        ...
        def test_soft_refresh_does_not_touch_coordinate_pipeline
          ...
          model.define_singleton_method(:enable_local_grid_coordinate_v2!) { calls << :enable; true }
          ...
          model.define_singleton_method(:align_cell_space_local_frame_local_grid_v2) do |_group|
            raise 'soft refresh must not evaluate axes'
          end
          model.define_singleton_method(:normalize_cell_space_local_grid_v2!) do |_group, **_options|
            raise 'soft refresh must not invoke LVN'
          end
          model.define_singleton_method(:recenter_cell_space_geometry_local_grid_v2) do |_group, **_options|
            raise 'soft refresh must not recenter geometry'
          end

          result = IndoorModel.instance_method(:soft_refresh_runtime_data_local_grid_v2).bind(model).call
        ...
        def build_dispatch_model(enabled:)
          ...
          model.define_singleton_method(:enable_local_grid_coordinate_v2!) do
            calls << :enable
            state = true
            true
          end
          model.define_singleton_method(:local_grid_coordinate_v2_enabled?) { state }
