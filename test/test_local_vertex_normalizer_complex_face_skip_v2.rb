...
module ULOL
  module Indoor3DGmlModeler
    module IndoorCore
      class LocalVertexNormalizerComplexFaceSkipV2Test < Minitest::Test
        ...
        def test_debug_profile_reports_skipped_instead_of_success
          model = FakeModel.new
          entity = FakeEntity.new([FakeFace.new([513])])
          instance = normalizer(model)
          instance.define_singleton_method(:validate_entity!) { |_entity| true }
          ...
          output, = capture_io do
            @result = instance.normalize(entity, debug: true)
          end

          assert_equal :skipped, @result.dig(:debug_profile, :status)
          assert_equal true,
                       @result.dig(:debug_profile, :normalization_skipped)
          assert_match(/\[LVN DEBUG\] PROFILE SKIPPED/, output)
          assert_empty model.calls
        ensure
          @result = nil
        end
