...
- 각 단계마다 Undo/Redo 테스트를 반드시 포함한다.
- 테스트 실패 시 다음 단계로 진행하지 말고 해당 단계 안에서 수정한다.
...
```text
indoor3d/classes/IndoorCore/services/SceneGroupGuard.rb
indoor3d/classes/IndoorCore/IndoorModel.rb
indoor3d/classes/IndoorCore/services/indoor_model/runtime_support.rb
```
...
```text
indoor3d/classes/IndoorCore/services/indoor_model/runtime_support.rb
indoor3d/classes/IndoorCore/services/indoor_model/observer_routing.rb
indoor3d/classes/IndoorCore/services/SceneGroupGuard.rb
```
...
```text
indoor3d/classes/IndoorCore/services/indoor_model/feature_lifecycle.rb
indoor3d/classes/IndoorCore/services/indoor_model/scene_groups.rb
indoor3d/classes/IndoorCore/services/indoor_model/runtime_support.rb
```
...
```text
indoor3d/classes/IndoorCore/services/FeatureRegistry.rb
```
...
```text
indoor3d/classes/IndoorCore/features/cell_space.rb
indoor3d/classes/IndoorCore/features/state.rb
indoor3d/classes/IndoorCore/features/transition.rb
```
...
```text
indoor3d/classes/IndoorCore/services/AttributeSerializer.rb
```
...
- [ ] `direct_child_of_root?`가 실제로 어떤 상황에서 호출되는지 확인한다.
- [ ] 필요하면 임시 debug helper를 추가했다가 최종 커밋 전 제거한다.
- [ ] `entity.parent`, `root_group.entities.parent`, `root_group.definition` 관계를 확인한다.
- [ ] 의도에 맞는 명확한 조건식으로 변경한다.
- [ ] 기존 방식 유지가 맞다면 주석으로 SketchUp parent 구조를 설명한다.
...
```text
indoor3d/classes/IndoorCore/services/RuntimeRestorer.rb
indoor3d/classes/IndoorCore/IndoorModel.rb
```
...
# 11단계. Undo/Redo 후 runtime refresh schedule 추가
...
```text
indoor3d/classes/IndoorCore/observers/model_observer.rb
indoor3d/classes/IndoorCore/services/indoor_model/runtime_support.rb
indoor3d/classes/IndoorCore/services/indoor_model/observer_routing.rb
```
...
indoor3d/classes/IndoorCore/services/indoor_model/observer_routing.rb
indoor3d/classes/IndoorCore/services/indoor_model/feature_lifecycle.rb
indoor3d/classes/IndoorCore/services/indoor_model/topology.rb
```
...
```text
indoor3d/classes/IndoorCore/IndoorModel.rb
indoor3d/classes/IndoorCore/observers/app_observer.rb
indoor3d/classes/IndoorCore/observers/model_observer.rb
indoor3d/core.rb
```
...
- [ ] `IndoorModel.for(model = Sketchup.active_model)` 메서드를 추가한다.
- [ ] `IndoorModel.current`는 기존 호환을 위해 `for(Sketchup.active_model)`을 호출하게 한다.
- [ ] `initialize`는 model 인자를 받을 수 있게 한다.
- [ ] AppObserver / ModelObserver에서 model을 받을 수 있는 곳은 `IndoorModel.for(model)`을 사용한다.
- [ ] 기존 호출부를 한 번에 모두 바꾸지 않는다.
- [ ] 현재 모델 기준 동작이 깨지지 않게 한다.
...
| 11 | Undo/Redo 후 runtime refresh schedule 추가 | [ ] | [ ] |
