...
> 처리 상태: CellSpace 복사본은 `duplicate_cell_space_identity?` 감지 후 `make_cell_space_copy_independent` 흐름에서 새 CellSpace/State id를 발급하고 transition id를 계승하지 않도록 이미 처리했다. Transition은 현재 runtime adjacency 산출물로 유지하며, refresh 시 새로 계산되는 동작은 일단 유지한다.
...
- [x] EditMode에서 CellSpace 복사/붙여넣기 시 어떤 observer callback이 발생하는지 확인한다.
- [x] 복사된 entity가 기존 IndoorGML attribute를 그대로 가진 상태로 들어오는지 확인한다.
- [x] 복사본을 새 CellSpace로 취급할지, 복사 자체를 막을지 정책을 결정한다.
- [x] 새 CellSpace로 취급하는 경우 기존 id/duality/transition 관련 attribute를 재생성한다.
- [x] 복사를 막는 경우 사용자에게 안전하게 되돌리거나 원본 외 복제본을 제거한다.
- [x] Export GML에 중복 CellSpace id가 나오지 않도록 한다.
