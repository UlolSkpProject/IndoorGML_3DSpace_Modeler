...
def replace_exact!(path, before, after)
  ...
  unless source.include?(before)
    raise "Expected cleanup fragment not found in #{path}: #{before.lines.first.inspect}"
  end
...
METHOD_RANGES.each do |filename, ranges|
  remove_line_ranges(File.join(LVN_DIR, filename), ranges)
end
...
replace_exact!(legacy, "        MAX_COLLINEAR_REPAIRS = 1_000\n", '')
replace_exact!(
  legacy,
  "        class << self\n\n          def normalized?",
  "        class << self\n          def normalized?"
...
raise 'Obsolete axis-plane merge test was not found' unless test_start && test_end
...
raise 'Obsolete surface descriptor fixture was not found' unless fixture_start && fixture_end
